...
const SIGLA_COLORS = {
  'A':    { bg: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20', label: 'Presente' },
  'I-OP': { bg: 'bg-indigo-500/15 text-indigo-400 border-indigo-500/20', label: 'Ingreso Operativo' },
  'FI':   { bg: 'bg-rose-500/15 text-rose-400 border-rose-500/20', label: 'Falta Injustificada' },
  'FJ':   { bg: 'bg-violet-500/15 text-violet-400 border-violet-500/20', label: 'Falta Justificada' },
  'B':    { bg: 'bg-slate-950 text-slate-600 border-slate-900', label: 'Baja' }
}
...
export default function AttendanceBI({ grupos = [], postulantes = [], asistencias = [] }) {
  ...
  const { biMetrics, motivesData, desertionTrend, recruiterData } = useMemo(() => {
    ...
    filteredAsistencias.forEach(a => {
      ...
      if (a.sigla_asistencia === 'B') dailyBajas[date]++
    })
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div>
        <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">
          Analítica BI & Retención
        </h2>
        <p className="text-slate-400 text-sm mt-1">
          Métricas consolidadas de deserción y rendimiento de reclutamiento.
        </p>
      </div>

      {/* Interactive Filters */}
      <div className="glass rounded-3xl p-5 border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl shadow-xl">
        <div className="flex items-center space-x-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-4 border-b border-slate-800 pb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Grupo */}
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Grupo de Capacitación</label>
            <select
              value={selectedGrupoCode}
              onChange={(e) => { setSelectedGrupoCode(e.target.value); setHeatmapPage(0) }}
              className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl px-4 py-2 text-xs text-slate-200 focus:outline-none transition-colors"
            >
          ...
          {/* Sede */}
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Sede</label>
            <select
              value={selectedSede}
          ...
          {/* Campaña */}
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Campaña</label>
            <select
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          const Icon = kpi.icon
          return (
            <div key={idx} className="glass rounded-3xl p-5 border border-slate-800/80 bg-slate-900/40 shadow-xl relative overflow-hidden group">
              <div className="flex justify-between items-start">
                ...
                <div className={\`p-2.5 bg-slate-950 rounded-xl \${kpi.color} border border-slate-800\`}>
                  <Icon size={16} />
                </div>
              </div>
              <div className="absolute bottom-0 left-0 w-full h-[3px] bg-slate-800/80 group-hover:bg-indigo-500 transition-colors" />
            </div>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass rounded-3xl p-6 border border-slate-800/80 bg-slate-900/40 shadow-xl">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Causas de Deserción</h4>
          </div>
          <div className="h-72">
            {motivesData.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-slate-550 text-xs text-center">
        ...
        <div className="glass rounded-3xl p-6 border border-slate-800/80 bg-slate-900/40 shadow-xl lg:col-span-2">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Curva de Deserción Acumulada</h4>
          </div>
          <div className="h-72">
            {desertionTrend.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={desertionTrend} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                  ...
                  <YAxis domain={[0, 'dataMax']} stroke="#64748b" fontSize={10} tickLine={false} />
                  <RechartsTooltip
                    contentStyle={{ background: '#090d16', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '12px' }}
      ...
      <div className="glass rounded-3xl p-6 border border-slate-800/80 bg-slate-900/40 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Matriz de Rendimiento por Reclutador</h4>
          </div>
          <p className="text-[10px] text-slate-500 mt-2 md:mt-0">Volumen Traído vs Tasa de Retención</p>
        </div>
        <div className="h-80">
          {recruiterData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" opacity={0.2} />
                <XAxis type="number" dataKey="Total" name="Volumen" stroke="#64748b" fontSize={10} tickLine={false} label={{ value: 'Volumen Reclutado', position: 'insideBottom', offset: -10, fill: '#64748b', fontSize: 10 }} />
                <YAxis type="number" dataKey="Retencion" name="Retención" unit="%" stroke="#64748b" fontSize={10} tickLine={false} domain={[0, 100]} label={{ value: 'Retención %', angle: -90, position: 'insideLeft', fill: '#64748b', fontSize: 10 }} />
      ...
      <div className="glass rounded-3xl border border-slate-800/80 bg-slate-900/40 shadow-2xl overflow-hidden">
        <div className="px-6 py-5 bg-slate-900/60 border-b border-slate-800/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 bg-indigo-500/10 text-indigo-400 rounded-xl border border-indigo-500/20">
              <Grid size={18} />
            </div>
            <div>
              <h4 className="font-bold text-slate-100 text-sm">Matriz de Asistencia (Heatmap)</h4>
              <p className="text-[10px] text-slate-500 mt-0.5">Seguimiento individual de asistencia a lo largo de la capacitación.</p>
            </div>
          </div>
          
          <div className="flex items-center gap-6">
            <div className="flex gap-3 text-[9px] font-bold text-slate-500 uppercase tracking-wider">
              ...
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-slate-950 border border-slate-900" /> B</span>
            </div>

            {totalPages > 1 && (
              <div className="flex items-center space-x-2 bg-slate-950 px-2 py-1.5 rounded-lg border border-slate-800">
                <button 
                  disabled={heatmapPage === 0} 
                  onClick={() => setHeatmapPage(p => p - 1)}
                  className="px-2 py-0.5 text-xs font-bold text-slate-300 disabled:opacity-30 hover:bg-slate-800 rounded"
                >
                  &lt;
                </button>
        ...
        <div className="table-scroll overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/40 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-800/60">
              <tr>
            ...
            <tbody className="divide-y divide-slate-850">
              {paginatedHeatmapRows.length > 0 ? (
                paginatedHeatmapRows.map(row => (
                  <tr key={row.documento} className="hover:bg-slate-900/10 transition-colors">
                    <td className="px-6 py-3.5">
                      <div className="font-semibold text-slate-200">{row.nombre}</div>
                      <div className="text-[9px] text-slate-550 font-mono mt-0.5">{row.documento}</div>
                    </td>
                    {heatmapData.dates.map(date => {
                      const sigla = row.dates[date] || '—'
                      const meta = SIGLA_COLORS[sigla] || { bg: 'bg-slate-900/40 text-slate-650 border border-slate-850', label: 'No Registrado' }
                      return (
                ...
                <tr>
                  <td colSpan={heatmapData.dates.length + 1} className="px-6 py-12 text-center text-slate-600 font-semibold">
...
fs.writeFileSync('/Users/saidaquino/Library/Mobile Documents/com~apple~CloudDocs/ARCHIVOS/IMPLEMENTACION NUEVO FRONT/GEAismael - FRONT/src/components/AttendanceBI.jsx', code)
console.log('Done')
