...
async function run() {
  ...
  try {
     console.log('Replacing sugerir_grupo_capacitacion...');
     await client.query(`
CREATE OR REPLACE FUNCTION public.sugerir_grupo_capacitacion(
  p_campana VARCHAR,
  p_semana INTEGER,
  p_periodo VARCHAR,
...
DECLARE
  v_grupo_codigo VARCHAR;
BEGIN
  IF NULLIF(TRIM(p_grupo_explicito), '') IS NOT NULL THEN
    SELECT codigo INTO v_grupo_codigo FROM capacidad_rys
    WHERE UPPER(codigo) = UPPER(TRIM(p_grupo_explicito))
      AND UPPER(campana) = UPPER(TRIM(p_campana))
    LIMIT 1;
    IF v_grupo_codigo IS NOT NULL THEN RETURN v_grupo_codigo; END IF;
    
    -- Fallback if campana doesn't match perfectly
    SELECT codigo INTO v_grupo_codigo FROM capacidad_rys
    WHERE UPPER(codigo) = UPPER(TRIM(p_grupo_explicito))
    LIMIT 1;
    IF v_grupo_codigo IS NOT NULL THEN RETURN v_grupo_codigo; END IF;
  ...
  SELECT g.codigo INTO v_grupo_codigo
  FROM capacidad_rys g
  WHERE UPPER(g.campana) = UPPER(TRIM(p_campana))
    AND g.semana_trabajo = p_semana
    AND (
      g.periodo = p_periodo
      OR g.periodo IS NULL
      OR p_periodo IS NULL
    )
  ORDER BY
    CASE WHEN g.periodo = p_periodo THEN 0 ELSE 1 END,
    CASE WHEN g.estado IN ('ACTIVO', 'EN_CURSO', 'PLANIFICADO') THEN 0 ELSE 1 END,
    g.created_at DESC
  LIMIT 1;

  RETURN v_grupo_codigo;
END;
$$;
     `);
...
BEGIN
  ...
  v_grupo_codigo := sugerir_grupo_capacitacion(
    v_campana,
    v_semana,
    v_periodo,
    p_data->>'grupo_codigo'
  ...
  INSERT INTO nominas (
    ...
    -- Datos Operativos
    reclutador_id, sede, campana, segmento,
