...
$asistSemana34Dates = @($asistData | Where-Object { 
    $d = Parse-Date $_.fecha_registro_asistencia
    $targetDates -contains $d
})

# 1. MotivosBajasBI (en las fechas de Semana 34)
$seenMB = @{}
$mbBajas = 0
foreach ($a in $asistSemana34Dates) {
...
# 2. ResumenCapacitacion (Curva de Deserción en esas fechas)
$seenRC = @{}
$rcBajas = 0
foreach ($a in $asistSemana34Dates) {
    $doc = "$($a.documento)".Trim()
...
# 3. AttendanceBI (Curva y métricas en esas fechas)
$seenAtt = @{}
$attBajas = 0
$attBajasD1 = 0
$seenD1 = @{}
...
# 4. ConsolidadoPowerBI (último estado en esas fechas)
$latestDoc = @{}
foreach ($a in $asistSemana34Dates) {
    $doc = "$($a.documento)".Trim()
    if (-not $doc) { continue }
...
Write-Output "=========================================================================================="
Write-Output "VERIFICACION CRUZADA DE BAJAS DE FORMACION EN FECHAS DE SEMANA 34 (22 a 25 Agosto 2026)"
Write-Output "=========================================================================================="
